...
From this lab's directory (`chapter-07/2-gitops-mcp/`):
...
git commit -m "chapter 7 lab 2: gitops-mcp + skill + hook"
...
Each section of Chapter 7, exercised end-to-end:
...
- *The recursive moment of Chapter 7*: the agent's own deployment is updated by the same ApplicationSet that ships my-first-app — the platform that deploys the agent is the platform the agent later consumes.
