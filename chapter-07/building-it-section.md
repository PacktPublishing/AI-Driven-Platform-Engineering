# Building It: Operational Agents in Production
...
A second `<SidebarItem>` in `Root.tsx` adds the `Platform Agent` entry to the chat sidebar. After a restart, the user sees two chat agents side by side — *Chat Assistant* (langgraph in-process, the Chapter 5 way) and *Platform Agent* (Strands HTTP proxy, the Chapter 7 way):
...
[^repo-ch6]: The reference implementation accompanying this section, with all five labs and the running demo, lives at <https://github.com/lusoal/ai-driven-platform-engineering/tree/main/chapter-07>.
